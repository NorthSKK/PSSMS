...
function formatThaiDate(dateString) {
  if (!dateString || dateString === "undefined" || dateString === "") return "ยังไม่กำหนด";
...
function renderApp(user) {
  ...
  if(user.currentTerm && user.currentYear) {
    termText = `<span class="badge bg-info text-dark shadow-sm" style="font-size: 0.7rem; border-radius: 6px;">เทอม ${user.currentTerm}/${user.currentYear}</span>`;
  }
  ...
  if (role === 'STUDENT') {
    ...
    menuHTML += `<li class="mb-3"><a href="javascript:void(0)" class="nav-link-custom active" onclick="loadPage('Page_Dashboard_Student')"><i class="fas fa-home me-2"></i>หน้าแรกนักเรียน</a></li>`;
  } else {
    startPage = (role === 'ADMIN') ? 'Page_Dashboard_Admin' : 'Page_Dashboard_Teacher';
    const homeLabel = (role === 'ADMIN') ? 'แดชบอร์ดผู้บริหาร' : 'แดชบอร์ดครู';
  ...
  if (role === 'ADMIN') {
    menuHTML += `
      <div class="menu-divider">การจัดการระบบ</div>
      <li class="mb-1"><a href="javascript:void(0)" class="nav-link-sub text-warning" onclick="loadPage('Page_Admin_Users')"><i class="fas fa-users-cog me-2"></i>จัดการผู้ใช้งาน</a></li>
      <li class="mb-1"><a href="javascript:void(0)" class="nav-link-sub text-info" onclick="loadPage('Page_Admin_Timetable')"><i class="fas fa-calendar-alt me-2"></i>จัดการตารางสอน</a></li>
      <li class="mb-3"><a href="javascript:void(0)" class="nav-link-sub text-light" onclick="loadPage('Page_Admin_Settings')"><i class="fas fa-cogs me-2"></i>ตั้งค่าระบบ</a></li>
    `;
  }

  if (role !== 'STUDENT') {
    menuHTML += `
      <div class="menu-divider">บริหารจัดการ 4 ฝ่าย</div>
      <li class="mb-1"><a href="javascript:void(0)" class="dept-btn academic" onclick="loadPage('Page_Academic')"><i class="fas fa-graduation-cap"></i> <span>ฝ่ายวิชาการ</span></a></li>
      <li class="mb-1 ms-3"><a href="javascript:void(0)" class="nav-link-sub text-success py-1" style="font-size: 0.85rem;" onclick="loadPage('Page_Score_Entry')"><i class="fas fa-edit me-2" style="font-size: 0.7rem;"></i>บันทึกคะแนน (ปพ.5)</a></li>
      <li class="mb-1 ms-3"><a href="javascript:void(0)" class="nav-link-sub text-info py-1" style="font-size: 0.85rem;" onclick="loadPage('Page_Academic_Report')"><i class="fas fa-chart-pie me-2" style="font-size: 0.7rem;"></i>รายงานสถิติ (มส.)</a></li>
      <li class="mb-2 ms-3"><a href="javascript:void(0)" class="nav-link-sub text-success py-1" style="font-size: 0.85rem;" onclick="loadPage('Page_Lesson_History')"><i class="fas fa-book-reader me-2" style="font-size: 0.7rem;"></i>แฟ้มบันทึกหลังสอน</a></li>
      <li class="mb-2"><a href="javascript:void(0)" class="dept-btn budget" onclick="loadPage('Page_Budget')"><i class="fas fa-wallet"></i> <span>ฝ่ายงบประมาณ</span></a></li>
      <li class="mb-2"><a href="javascript:void(0)" class="dept-btn personnel" onclick="loadPage('Page_Personnel')"><i class="fas fa-users"></i> <span>ฝ่ายบุคคล</span></a></li>
      <li class="mb-2"><a href="javascript:void(0)" class="dept-btn general" onclick="loadPage('Page_General')"><i class="fas fa-building"></i> <span>ฝ่ายทั่วไป</span></a></li>
    `;
  }
...
function loadPage(pageName) {
  ...
  showLoader(true);
  google.script.run.withFailureHandler(err => { showLoader(false); alert("โหลดหน้าเว็บไม่สำเร็จ: " + err.message); }).withSuccessHandler(function(html) {
...
function renderTodoList(todos) {
  ...
  if (!todos || todos.length === 0) {
    container.innerHTML = `<div class="d-flex align-items-center justify-content-center w-100 animate__animated animate__fadeIn" style="min-height: 100px; background-color: #ffffff; border: none;"><div class="text-center"><i class="fas fa-check-double fa-lg mb-2 text-success" style="opacity: 0.12;"></i><br><span class="text-muted" style="font-size: 0.82rem; opacity: 0.4; letter-spacing: 0.5px;">เคลียร์งานหมดแล้วครับ 🎉</span></div></div>`; 
...
function renderTeacherTodaySchedule(timetable) {
  ...
  timetable.forEach(r => {
    ...
    html += `<tr class="${isTeaching ? 'table-primary' : ''}" style="cursor: pointer; transition: background-color 0.2s;" onmouseover="this.style.backgroundColor='#f1f3f5'" onmouseout="this.style.backgroundColor=''" onclick="jumpToAttendance('${encodedItem}')" title="👆 คลิกเพื่อไปหน้าเช็คชื่อวิชานี้"><td class="ps-3 fw-bold align-middle">คาบ ${p}</td><td class="align-middle"><div class="fw-bold text-primary">${r[0]} - ${r[1]}</div><div class="small text-muted"><i class="fas fa-door-open me-1"></i>ห้อง ${r[2]} (${r[4]}) | <i class="fas fa-clock me-1"></i>${timeRange} น.</div></td><td class="text-center align-middle">${isTeaching ? '<span class="badge bg-danger pulse mb-1">กำลังสอน</span>' : (startTotalMinutes > currentMinutes ? '<span class="badge bg-light text-dark border mb-1">รอสอน</span>' : '<span class="badge bg-success mb-1">สอนเสร็จแล้ว</span>')}<br><span class="badge bg-primary rounded-pill" style="font-size: 0.65rem;"><i class="fas fa-hand-pointer me-1"></i>คลิกเช็คชื่อ</span></td></tr>`;
  });
  tbody.innerHTML = html;
  if(nextTitle && nextClass) {
    nextTitle.innerHTML = `คาบ ${nextClass[5]}: <span class="text-warning">${nextClass[0]}</span> ${nextClass[1]}`;
...
function loadTeacherAtRiskDashboard(teacherId, term, year) {
  ...
  container.innerHTML = `<div class="text-center p-4"><div class="spinner-border text-danger mb-2" role="status"></div><div class="text-muted small">กำลังสแกนหานักเรียนกลุ่มเสี่ยงจากทุกรายวิชา...</div></div>`;
  google.script.run.withSuccessHandler(function(data) {
...
function buildTimetableDropdown(timetable) {
  ...
  timetable.forEach(item => {
    ...
    opt.text = `คาบ ${item[5]}: ${item[0]} ${item[1]} (${String(item[2]).trim()})`;
    select.add(opt);
  });
...
function loadStudentList(forceRefresh = false) {
    ...
    if(document.getElementById('dispClass')) document.getElementById('dispClass').innerText = `กำลังดึงรายชื่อห้อง: ${fullClassName}`;
    ...
    const fetchHistoryAndRender = (studentsData) => {
        ...
        .withSuccessHandler(function(historyArray) {
            ...
            if(btn) {
                ...
                if(!detailBtn) btn.insertAdjacentHTML('afterend', `<button id="btnDetailedSubmit" class="btn btn-outline-success w-100 py-2 mt-2 fw-bold shadow-sm" onclick="openDetailedLessonModal()"><i class="fas fa-clipboard-list me-2"></i>จบบทเรียน: บันทึกหลังสอนแบบละเอียด (DPA/3R8C)</button>`);
            }
    ...
    if (!forceRefresh && sessionStorage.getItem(cacheKey)) {
        ...
        .withSuccessHandler(function(students) {
            ...
            if(!students || students.length === 0) {
                showLoader(false);
                document.getElementById('studentRows').innerHTML = `<tr><td colspan="3" class="text-center p-4 text-danger">ไม่พบรายชื่อนักเรียนห้อง ${fullClassName}</td></tr>`;
...
function submitAcademicData() {
  ...
  if(!topic) return alert("กรุณาระบุหัวข้อที่สอนด้วยครับ");
  const sigData = sigCanvas.toDataURL();
  if(sigData.length < 1000) return alert("กรุณาลงชื่อผู้สอนก่อนบันทึกครับ");
  ...
  const lessonRecord = {
    ...
    totalPresent: parseInt(document.getElementById('sumPresent').innerText) + parseInt(document.querySelectorAll('input[value="สาย"]:checked').length),
    totalAbsent: document.getElementById('sumAbsent').innerText,
    totalLeave: document.querySelectorAll('input[value="ลา"]:checked').length,
  ...
  google.script.run.withSuccessHandler(function() {
    google.script.run.withSuccessHandler(function() {
      ...
      alert("✅ บันทึกข้อมูลเช็คชื่อและผลการสอนเรียบร้อยแล้ว");
...
// ----------------------------------------------------------------------
// ชุดฟังก์ชัน Modal ประวัติย้อนหลัง
// ----------------------------------------------------------------------
function viewTodayHistory() {
  const classVal = document.getElementById('selectClass').value;
  const dateInput = document.getElementById('teachingDate'); 
  if(!classVal) return alert("กรุณาเลือกวิชาที่สอนก่อนครับ");
  ...
  google.script.run.withSuccessHandler(function(history) {
    showLoader(false);
    if (!history || history.length === 0) return alert("วันที่เลือกยังไม่มีการบันทึกข้อมูลครับ");
...
function loadHistoryForEdit(dateStr) {
  ...
  google.script.run.withSuccessHandler(function(historyArray) {
    ...
    if (!tempHistoryData || tempHistoryData.length === 0) return alert("ไม่พบข้อมูลรายละเอียด");
    renderHistoryModal(subjectCode);
  }).getTodayAttendanceHistory(dateStr, subjectCode, className);
}
...
function saveAllHistoryChanges() {
  if(!confirm("ยืนยันการบันทึกการแก้ไขทั้งหมดใช่หรือไม่?")) return;
  const user = getSessionUser();
  showLoader(true);
  google.script.run.withSuccessHandler(function(res) {
    showLoader(false);
...
// ==========================================
// ระบบบันทึกหลังสอนแบบละเอียด (Detailed Lesson)
// ==========================================
function openDetailedLessonModal() {
  const topic = document.getElementById('inputTopic').value;
  if(!topic) { alert("กรุณากรอก 'หัวข้อที่สอน' ก่อนกดบันทึกแบบละเอียดครับ"); document.getElementById('inputTopic').focus(); return; }
  const sigData = sigCanvas.toDataURL();
  if(sigData.length < 1000) { alert("กรุณาลงชื่อผู้สอนด้านบนให้เรียบร้อยก่อนครับ"); return; }
...
async function submitDetailedLesson() {
  ...
  submitBtn.disabled = true; submitBtn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>กำลังอัปโหลดไฟล์และบันทึก...';
  showLoader(true);
  ...
  try {
    ...
    const basicRecord = {
      date: date, subjectCode: item[0], subjectName: item[1], className: normalizedClassName, period: item[5], topic: topic,
      totalPresent: parseInt(document.getElementById('sumPresent').innerText) + parseInt(document.querySelectorAll('input[value="สาย"]:checked').length),
      totalAbsent: document.getElementById('sumAbsent').innerText, totalLeave: document.querySelectorAll('input[value="ลา"]:checked').length,
    ...
    showLoader(false); submitBtn.disabled = false; submitBtn.innerHTML = originalBtnText; alert("เกิดข้อผิดพลาด: " + err.message);
  }
}
...
function reloadReportSubjects() {
  ...
  select.innerHTML = `<option value="">⏳ กำลังโหลดวิชา...</option>`; select.disabled = true;
  const tbody = document.getElementById('reportTableBody'); if(tbody) tbody.innerHTML = '<tr><td colspan="7" class="text-center p-4 text-muted">กรุณาเลือกวิชา</td></tr>';
  ...
  google.script.run
    .withFailureHandler(err => { select.innerHTML = '<option value="">❌ เกิดข้อผิดพลาด</option>'; select.disabled = false; alert(err.message); })
...
function buildReportSubjectDropdown(subjects, term, year) {
  ...
  if(subjects.length === 0) { select.innerHTML = `<option value="">⚠️ ไม่พบวิชาในเทอม ${term}/${year}</option>`; return; }
...
function generateReport() {
  ...
  if(!val) return alert("กรุณาเลือกรายวิชาก่อนครับ");
  ...
  if (val === "ALL") {
    ...
    } catch (e) { showLoader(false); alert("ข้อมูลรายวิชาไม่ถูกต้อง: " + e.message); }
  }
}
...
function renderReportTable(res, subjectCode, subjectName, className) {
  ...
  if(!res || !res.students || res.students.length === 0) { tbody.innerHTML = '<tr><td colspan="7" class="text-center p-4 text-muted">ไม่พบข้อมูลการเช็คชื่อในรายวิชานี้</td></tr>'; return; }
  ...
  if(tableEl) { tableEl.insertAdjacentHTML('beforebegin', infoHtml); const theads = tableEl.querySelectorAll('th'); if (theads.length >= 3) { theads[1].innerText = "รหัสนักเรียน"; theads[2].innerText = "ชื่อ-สกุล"; } }
  ...
  data.forEach((s, index) => {
    ...
    if (remainingQuota >= 0) quotaText = `<span class="text-muted small">ขาดได้อีก <b>${remainingQuota}</b> ครั้ง</span>`; else quotaText = `<span class="text-danger fw-bold small">เกินโควตา <b>${Math.abs(remainingQuota)}</b> ครั้ง</span>`;
    if (p > 85) { statusBadge = '<span class="badge bg-success rounded-pill px-2">ปกติ</span>'; progressColor = 'bg-success'; } else if (p >= 80) { statusBadge = '<span class="badge bg-info text-dark rounded-pill px-2">เฝ้าระวัง</span>'; progressColor = 'bg-info'; } else if (p >= 60) { statusBadge = '<span class="badge bg-warning text-dark rounded-pill px-2">มส.</span>'; rowClass = 'table-warning'; progressColor = 'bg-warning'; } else { statusBadge = '<span class="badge bg-danger rounded-pill px-2">หมดสิทธิ์</span>'; rowClass = 'table-danger'; progressColor = 'bg-danger'; }
...
function renderAllSubjectsReportTable(data) {
  ...
  tbody.innerHTML = '';
  const oldHeader = document.getElementById('reportSummaryInfo'); if(oldHeader) oldHeader.remove();
  if(!data || data.length === 0) { tbody.innerHTML = '<tr><td colspan="7" class="text-center p-4 text-muted">ไม่พบข้อมูลการเช็คชื่อในเทอมนี้</td></tr>'; return; }
  ...
  data.forEach((s) => {
    ...
    let statusBadge = ''; let rowClass = ''; let progressColor = ''; let quotaText = '';
    if (remainingQuota >= 0) quotaText = `<span class="text-muted small">เหลือโควตา <b>${remainingQuota}</b></span>`; else quotaText = `<span class="text-danger fw-bold small">เกิน <b>${Math.abs(remainingQuota)}</b></span>`;
    if (p > 85) { statusBadge = '<span class="badge bg-success rounded-pill px-2">ปกติ</span>'; progressColor = 'bg-success'; } else if (p >= 80) { statusBadge = '<span class="badge bg-info text-dark rounded-pill px-2">เฝ้าระวัง</span>'; progressColor = 'bg-info'; } else if (p >= 60) { statusBadge = '<span class="badge bg-warning text-dark rounded-pill px-2">มส.</span>'; rowClass = 'table-warning'; progressColor = 'bg-warning'; } else { statusBadge = '<span class="badge bg-danger rounded-pill px-2">หมดสิทธิ์</span>'; rowClass = 'table-danger'; progressColor = 'bg-danger'; }
...
function deleteLessonHistory(encodedTs) {
  if(!confirm("⚠️ ยืนยันการลบบันทึกหลังสอนรายการนี้ใช่หรือไม่? (ลบแล้วไม่สามารถกู้คืนได้)")) return;
...
function openEditLessonHistory(encodedTs) {
  const ts = decodeURIComponent(encodedTs); const item = allLessonHistories.find(i => String(i.timestamp) === ts); if(!item) return alert("ไม่พบข้อมูล");
...
function initSubjectConfigPage() {
  ...
  if (cachedSubjects && cachedSubjects !== "[]") { buildConfigSubjectDropdown(JSON.parse(cachedSubjects)); } 
     ...
     google.script.run.withFailureHandler(err => { select.innerHTML = `<option value="">❌ โหลดข้อมูลล้มเหลว</option>`; alert("Error: " + err.message); })
...
function saveSubjectConfigData() {
  const val = document.getElementById('configClassSelect').value;
  if(!val) return alert("กรุณาเลือกวิชา");
  ...
  google.script.run.withSuccessHandler(res => { btn.innerHTML = '<i class="fas fa-save me-2"></i>บันทึกโครงสร้างวิชา'; btn.disabled = false; alert(res.message); }).saveSubjectConfig(configData);
}
...
// 🌟 กุญแจมาตรฐาน (เกราะป้องกันรหัสเด็ก)
...
function toggleHeaderEditMode() {
  ...
  if (!isHeaderEditMode) {
    ...
    btnEdit.innerHTML = '<i class="fas fa-check me-2"></i>เสร็จสิ้นการแก้หัวตาราง';
    refreshTableOnly();
  } else {
    if(confirm("💡 คุณอาจมีการปรับเปลี่ยนโครงสร้างหรือตัวชี้วัด\nต้องการบันทึกข้อมูลลงระบบตอนนี้เลยหรือไม่?")) {
       saveAllInOneScores(); 
    } else {
...
function toggleQualColumns() {
  ...
  if (isQualVisible) {
    ...
    btn.innerHTML = '<i class="fas fa-eye-slash me-2"></i>ซ่อนคุณลักษณะ';
  } else {
    btn.className = "btn btn-outline-info shadow-sm px-3 fw-bold me-2";
    btn.innerHTML = '<i class="fas fa-eye me-2"></i>ประเมินคุณลักษณะ';
  }
  syncHeaderState(); 
  refreshTableOnly(); 
}
...
// 🌟 2. ฟังก์ชันวาดตาราง (ฉบับอัปเกรดความกระชับ & นำ % ไปต่อท้ายรหัส)
function renderAllInOneTable(students, config, existingScores, existingQuals) {
  ...
  // 🌟 ปรับขนาดและลด Padding ของคอลัมน์แรกๆ
  let headHtml = `<tr><th rowspan="2" class="align-middle text-center sticky-head-1 text-muted px-1" style="width:40px;">ที่</th><th rowspan="2" class="align-middle text-start sticky-head-2 text-muted px-2" style="min-width: 170px;">ชื่อ-สกุล</th>`;
  let subHeadHtml = `<tr>`;
  ...
  headHtml += `<th class="text-primary border-bottom-0 border-start col-highlight-f">รวมเก็บ</th><th class="text-warning text-dark border-bottom-0 col-highlight-m">กลางภาค</th><th class="text-danger border-bottom-0 border-start border-white col-highlight-mr" style="opacity: 0.8;">ซ่อม</th><th class="text-danger border-bottom-0 border-start col-highlight-e">ปลายภาค</th><th class="text-dark border-bottom-0 border-start col-highlight-total">รวม</th><th class="text-success border-bottom-0 border-start col-highlight-grade">เกรด</th><th class="text-danger border-bottom-0 border-start col-highlight-mr">หมายเหตุ</th><th colspan="3" class="text-info text-dark border-start ${isQualVisible ? '' : 'd-none'}">ประเมิน (0-3)</th></tr>`;
  
  subHeadHtml += `<th class="text-primary small fw-normal border-start col-highlight-f">(<span id="edit-ratio-f">${ratios[0] || 0}</span>)</th><th class="text-warning text-dark small fw-normal col-highlight-m">(<span id="edit-ratio-m" contenteditable="${isHeaderEditMode}" onblur="reCalcAllRows()">${ratios[1] || 0}</span>)</th><th class="text-danger small fw-normal border-start border-white col-highlight-mr" style="opacity: 0.8;">-</th><th class="text-danger small fw-normal border-start col-highlight-e">(<span id="edit-ratio-e" contenteditable="${isHeaderEditMode}" onblur="reCalcAllRows()">${ratios[2] || 0}</span>)</th><th class="text-dark small fw-normal border-start col-highlight-total">(<span id="edit-ratio-total">100</span>)</th><th class="text-success small fw-normal border-start col-highlight-grade">0-4</th><th class="text-danger small fw-normal border-start col-highlight-mr">(ร, มส)</th><th class="text-info small fw-normal border-start ${isQualVisible ? '' : 'd-none'}">อ่าน</th><th class="text-info small fw-normal ${isQualVisible ? '' : 'd-none'}">คุณฯ</th><th class="text-info small fw-normal ${isQualVisible ? '' : 'd-none'}">สมรรถนะ</th></tr>`;
  ...
  students.forEach((std, rowIndex) => {
    ...
    if (pct !== undefined) {
        // 🌟 ย้าย % มาอยู่ในบรรทัดเดียวกับรหัส และทำป้ายให้สวยงาม
    ...
    // 🌟 จัดโครงสร้าง HTML ของเซลล์ชื่อนักเรียนใหม่ให้ชิดและสวยงาม
    bodyHtml += `<tr>
...
function saveAllInOneScores(isSilent = false) {
  ...
  if((f + m + e) !== 100) {
     if(!isSilent) {
        if(!confirm("⚠️ สัดส่วนคะแนนรวมกันไม่เท่ากับ 100 ต้องการบันทึกต่อหรือไม่?")) { isSavingScores = false; return; }
     } else {
  ...
  try {
      ...
      google.script.run
        .withFailureHandler(err => {
          ...
          if(!isSilent) alert("❌ ขัดข้องที่ระบบเซิร์ฟเวอร์: " + err.message);
        })
      ...
      isSavingScores = false; 
      if(btn) { btn.disabled = false; btn.innerHTML = originalText; btn.className = originalBtnClass; }
      if(!isSilent) alert("❌ UI Error: " + e.message);
      console.error(e);
  }
}

function addIndicator() {
  currentScoreConfig.indicators.push({ name: "งานใหม่", score: 10 });
  refreshTableOnly();
}
